...
def main_abnormal_dataset(
    ...
    data_input = '/mnt/pfs_l2/jieti_team/SFT/hupeng/data/en/audio_detect/test/label_only_abnormal_v2.csv'
    ...
    for idx, row in tqdm(data.iterrows(), total=len(data), desc="Inference", disable=False):
        ...
        start_time = time.time()
        text, text_probs = inference(model, input_text, infer_audio_content, max_new_tokens=1)
        end_time = time.time()
    ...
    fo.close()
    if infer_type == 'score':
        infer_statistic(rtf_list, infer_time_list, total_duration, 
        statistic_fo)
...
def main_asr(model_path:str, infer_data:str, fo_path:str):
    ...
    infer_df = pd.read_csv(infer_data, sep='\t')
    for _, row in tqdm(infer_df.iterrows(), total=len(infer_df), desc="ASR Inference", disable=False):
        key = row['wavname']
        ori_text = row['text']
        infer_audio_content = row['wavpath']
        audio_duration = (librosa.get_duration(filename=infer_audio_content, sr=16000)) * 1000  # ms
...
if __name__ == "__main__":
    ...
    # 异常音频测试集
    # main_abnormal_dataset(model_path, 'score') # audio_type、score
    
    # 励步
    # main_single_dataset(
    #     model_path=model_path,
    #     infer_file='/mnt/pfs_l2/jieti_team/SFT/hupeng/data/en/libu/test/text',
    #     audio_file='/mnt/pfs_l2/jieti_team/SFT/hupeng/data/en/libu/test/wavpath'
    # )
    # main_single_data()
